...
HTML_TEMPLATE: str = (
    "author_template.html"  # HTML template to use for the author page
)
...
class BaseSubstackScraper(ABC):
    def __init__(
        self, base_substack_url: str, md_save_dir: str, html_save_dir: str
    ):
    ...
    def fetch_urls_from_sitemap(self) -> List[str]:
        ...
        if not response.ok:
            print(
                f"Error fetching sitemap at {sitemap_url}: {response.status_code}"
            )
            return []
    ...
    def filter_urls(urls: List[str], keywords: List[str]) -> List[str]:
        ...
        return [
            url
            for url in urls
            if all(keyword not in url for keyword in keywords)
        ]
            ...
            try:
                ...
                if not os.path.exists(md_filepath):
                    ...
                    if soup is None:
                        total += 1
                        continue
                    title, subtitle, like_count, date, md = (
